...
func TestVideoRetriesExplicitAntiBotSubmitWithoutRepeatingUploads(t *testing.T) {
	dataURI := "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII="
	var uploadCalls atomic.Int64
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		switch request.URL.Path {
		case "/http/upload-file-v2/direct":
			index := uploadCalls.Add(1)
			writer.Header().Set("Content-Type", "application/json")
			_, _ = fmt.Fprintf(writer, `{"fileMetadata":{"fileMetadataId":"metadata-%d"}}`, index)
		case "/rest/app-chat/conversations/new":
			...
			writer.Header().Set("Content-Type", "text/event-stream")
			_, _ = io.WriteString(writer, `data: {"result":{"response":{"streamingVideoGenerationResponse":{"progress":100,"videoUrl":"users/test/generated/video.mp4"}}}}`+"\n")
		default:
			http.NotFound(writer, request)
		}
	}))
	...
	adapter, credential := newVideoProtocolTestAdapter(t, server.URL)
	result, err := adapter.GenerateVideo(context.Background(), provider.VideoRequest{
		Credential: credential, JobID: "video_retry_submit", Prompt: "一起跳舞", Duration: 10,
		AspectRatio: "2:3", Resolution: "720p", ReferenceURLs: []string{dataURI, dataURI},
	})
	if err != nil {
		t.Fatal(err)
	}
	if result.URL != "https://assets.grok.com/users/test/generated/video.mp4" || uploadCalls.Load() != 2 || submitCalls.Load() != 2 {
		t.Fatalf("result=%#v upload_calls=%d submit_calls=%d", result, uploadCalls.Load(), submitCalls.Load())
	}
}

func TestVideoRetriesExplicitAntiBotUpload(t *testing.T) {
	dataURI := "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII="
	var uploadCalls atomic.Int64
	var submitCalls atomic.Int64
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/http/upload-file-v2/direct":
			...
			writer.Header().Set("Content-Type", "application/json")
			_, _ = io.WriteString(writer, `{"fileMetadata":{"fileMetadataId":"metadata-1"}}`)
		case "/rest/app-chat/conversations/new":
			submitCalls.Add(1)
			writer.Header().Set("Content-Type", "text/event-stream")
			_, _ = io.WriteString(writer, `data: {"result":{"response":{"streamingVideoGenerationResponse":{"progress":100,"videoUrl":"users/test/generated/video.mp4"}}}}`+"\n")
	...
	adapter, credential := newVideoProtocolTestAdapter(t, server.URL)
	result, err := adapter.GenerateVideo(context.Background(), provider.VideoRequest{
		Credential: credential, JobID: "video_retry_upload", Prompt: "跳舞", Duration: 10,
		AspectRatio: "2:3", Resolution: "720p", ReferenceURLs: []string{dataURI},
	})
	if err != nil {
		t.Fatal(err)
	}
	if result.URL == "" || uploadCalls.Load() != 2 || submitCalls.Load() != 1 {
		t.Fatalf("result=%#v upload_calls=%d submit_calls=%d", result, uploadCalls.Load(), submitCalls.Load())
	}
}

func TestVideoDoesNotRetryOrdinaryForbiddenSubmit(t *testing.T) {
	dataURI := "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII="
	var uploadCalls atomic.Int64
	var submitCalls atomic.Int64
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/rest/app-chat/conversations/new":
			...
			writer.Header().Set("Content-Type", "application/json")
			writer.WriteHeader(http.StatusForbidden)
			_, _ = io.WriteString(writer, `{"error":{"code":"forbidden","message":"access denied"}}`)
		default:
			http.NotFound(writer, request)
		}
	}))
	...
	_, err := adapter.GenerateVideo(context.Background(), provider.VideoRequest{
		Credential: credential, JobID: "video_no_retry", Prompt: "跳舞", Duration: 10,
		AspectRatio: "2:3", Resolution: "720p", ReferenceURLs: []string{dataURI},
	})
	if err == nil || uploadCalls.Load() != 1 || submitCalls.Load() != 1 || isWebMediaAntiBotError(err) {
		t.Fatalf("error=%v upload_calls=%d submit_calls=%d", err, uploadCalls.Load(), submitCalls.Load())
	}
}

func TestTextVideoRetriesExplicitAntiBotMediaPost(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case "/rest/media/post/create":
			...
			writer.Header().Set("Content-Type", "application/json")
			_, _ = io.WriteString(writer, `{"post":{"id":"post-1"}}`)
		case "/rest/app-chat/conversations/new":
			submitCalls.Add(1)
			writer.Header().Set("Content-Type", "text/event-stream")
			_, _ = io.WriteString(writer, `data: {"result":{"response":{"streamingVideoGenerationResponse":{"progress":100,"videoUrl":"users/test/generated/video.mp4"}}}}`+"\n")
	...
	adapter, credential := newVideoProtocolTestAdapter(t, server.URL)
	result, err := adapter.GenerateVideo(context.Background(), provider.VideoRequest{
		Credential: credential, JobID: "text_video_retry", Prompt: "跳舞", Duration: 10, AspectRatio: "2:3", Resolution: "720p",
	})
	if err != nil {
		t.Fatal(err)
	}
	if result.URL == "" || postCalls.Load() != 2 || submitCalls.Load() != 1 {
		t.Fatalf("result=%#v post_calls=%d submit_calls=%d", result, postCalls.Load(), submitCalls.Load())
	}
}

func TestVideoStopsAfterSecondExplicitAntiBotSubmit(t *testing.T) {
	dataURI := "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII="
	var uploadCalls atomic.Int64
	var submitCalls atomic.Int64
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
	...
	adapter, credential := newVideoProtocolTestAdapter(t, server.URL)
	_, err := adapter.GenerateVideo(context.Background(), provider.VideoRequest{
		Credential: credential, JobID: "video_retry_exhausted", Prompt: "跳舞", Duration: 10,
		AspectRatio: "2:3", Resolution: "720p", ReferenceURLs: []string{dataURI},
	})
	if !isWebMediaAntiBotError(err) || uploadCalls.Load() != 1 || submitCalls.Load() != 2 {
		t.Fatalf("error=%v upload_calls=%d submit_calls=%d", err, uploadCalls.Load(), submitCalls.Load())
	}
}

func newVideoProtocolTestAdapter(t *testing.T, baseURL string) (*Adapter, account.Credential) {
	t.Helper()
	cipher, err := security.NewCipher(base64.StdEncoding.EncodeToString(make([]byte, 32)))
	if err != nil {
		t.Fatal(err)
	...
	adapter := NewAdapter(Config{BaseURL: baseURL, StatsigMode: "manual"}, infraegress.NewManager(egressRepositoryStub{}, cipher), cipher, nil, nil)
	return adapter, account.Credential{ID: 1, EncryptedAccessToken: encrypted}
}
