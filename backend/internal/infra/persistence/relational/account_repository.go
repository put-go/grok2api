...
func (r *AccountRepository) getRoutingQuotaWindows(ctx context.Context, provider account.Provider, quotaMode string, credentials []account.Credential) (map[uint64]account.QuotaWindow, error) {
	...
	// expose a product-specific remainingQueries window; load both and prefer
	// the exact product window below, falling back to weekly for paid tiers.
	if provider == account.ProviderWeb {
	...
	for _, row := range rows {
		if webImagineMode {
			...
			// The Usage-page weekly pool is authoritative for paid Imagine
			// products. Image-edit is the exception: its dedicated product
			// window remains authoritative for Super/Heavy accounts.
