...
func (s *Service) Update(ctx context.Context, id uint64, input UpdateInput) (View, error) {
	...
	if input.MaxConcurrent != nil {
		...
		value.MaxConcurrentManaged = false
	}
...
func (s *Service) resolveRefreshedQuotaWindow(ctx context.Context, id uint64, mode string, refreshed quotaRefreshResult) (accountdomain.QuotaWindow, error) {
	...
	sharedWebImagine := credential.Provider == accountdomain.ProviderWeb && isWebImagineQuotaMode(mode)
	if sharedWebImagine {
		if window, ok := quotaWindowByMode(refreshed.Windows, "weekly"); ok {
			return window, nil
		}
	}
	paidWebImagine := sharedWebImagine && (credential.WebTier == accountdomain.WebTierSuper || credential.WebTier == accountdomain.WebTierHeavy)
	if paidWebImagine {
