...
func (s *Selector) acquireSegmentedCandidates(ctx context.Context, values []account.RoutingCandidate, indexes []int, quotaMode string, tierPolicy any, request segmentedSelectorActiveRequest, materialFailures *credentialMaterialFailureTracker) (*accountLease, error) {
	...
	for {
		...
		if fullPlannerOnly {
			...
			cohorts := segmentedCandidateCohorts(values, indexes, now, tierGroups, preferFreeBuild, int(request.cursor), request.windowSize, segmentedWindowsBeforeFullFallback)
			roundWindows := 0
...
func segmentedCandidateCohorts(values []account.RoutingCandidate, indexes []int, now time.Time, tierPolicy any, preferFreeBuild bool, windowOptions ...int) []segmentedSelectorCohortBucket {
	tierGroups := normalizeTierGroups(tierPolicy)
	...
	if len(windowOptions) == 0 {
		buckets := make(map[segmentedSelectorCohort][]int, len(ordered))
		...
		result := make([]segmentedSelectorCohortBucket, 0, len(ordered))
		for _, cohort := range ordered {
			result = append(result, segmentedSelectorCohortBucket{cohort: cohort, indexes: buckets[cohort]})
		}
		return result
	}
	if len(windowOptions) != 3 {
		return nil
	}
	cursor, windowSize, maxWindows := windowOptions[0], windowOptions[1], windowOptions[2]
	...
	for _, cohort := range ordered {
		...
		selection := &segmentedCohortSelection{
			count: count, start: start, take: take, indexes: make([]int, take),
		}
