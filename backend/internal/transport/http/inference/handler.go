...
func (h *Handler) handleVideoCreate(c *gin.Context, operation, label string) {
	...
	if operation == gatewayVideoOperationGenerate {
		...
		if len(referenceURLs) > mediadomain.MaxReferenceImages {
			writeOpenAIError(c, http.StatusBadRequest, "invalid_request", fmt.Sprintf("reference_images 不能超过 %d 张", mediadomain.MaxReferenceImages))
			return
		}
		if len(referenceURLs) > 1 && duration > mediadomain.MaxReferenceVideoDuration {
