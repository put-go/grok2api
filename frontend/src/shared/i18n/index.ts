  ...
  "zh-CN": {
    translation: {
      ...
      docs: {
        ...
        conversationPinning: "绑定账号不可用时返回 upstream_pinned_account_unavailable，不会切换账号制造无效状态链。无状态回放可请求 reasoning.encrypted_content 并在后续 input 中原样带回。",
        ...
        privacyBehaviorDescription: "成功请求通常只保存路由、状态、耗时与 usage 元数据；若 adapter 在内部恢复了上游失败，会额外保存经过脱敏和容量限制的失败诊断。请求正文不会直接写入审计，encrypted reasoning 与 opaque 压缩状态会被剔除。",
        requestFields: "常用字段",
  ...
  en: {
    translation: {
      ...
      docs: {
        ...
        conversationPinning: "If the owning account is unavailable, the gateway returns upstream_pinned_account_unavailable instead of switching accounts and corrupting the state chain. Stateless replay can request reasoning.encrypted_content and pass it back in later input.",
        ...
        privacyBehaviorDescription: "Successful requests normally store routing, status, latency, and usage metadata only. When an adapter internally recovers an upstream failure, the audit also retains a redacted, size-limited failure diagnostic. Request bodies are not written directly, and encrypted reasoning or opaque compaction state is removed.",
