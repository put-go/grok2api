...
const resources = {
  "zh-CN": {
    translation: {
      ...
      models: {
        title: "模型路由",
        description: "同一对外模型名称可映射到 Grok Build、Grok Web 与 Grok Console 的不同上游路由；多个对外名称也可指向同一上游模型。",
        sync: "同步模型",
      ...
      keys: {
        ...
        modelAliasesBuildSupport: "Grok Build：grok-4.5、grok-3-mini、grok-3-mini-fast 支持 low / medium / high。",
        modelAliasesConsoleSupport: "Grok Console：grok-4.3 支持 none / low / medium / high；grok-4.20-0309-reasoning 支持 low / medium / high；grok-4.20-multi-agent-0309 额外支持 xhigh。",
      ...
      audits: {
        ...
        billingStoredFormulaUnavailable: "该记录使用历史或未知费率版本，仅保留最终估算值，无法用当前费率可靠重建。",
      ...
      settings: {
        ...
        routing: { title: "路由策略", stickyTTL: "会话粘性时长", stickyTTLHelp: "同一会话优先复用已选账号的有效时长；到期后重新参与调度。", cooldownBase: "基础冷却时间", cooldownBaseHelp: "账号发生可恢复故障后首次进入冷却的时长。", cooldownMax: "最大冷却时间", cooldownMaxHelp: "连续故障触发指数退避时允许达到的最长冷却时间。", capacityWait: "账号满载等待", capacityWaitHelp: "全部候选账号并发已满时，切换或失败前等待容量释放的时间。", maxAttempts: "最大尝试次数", maxAttemptsHelp: "单次请求在可用账号之间切换并重试的最大次数，范围为 1–200。", preferFreeBuild: "Grok Build Free 账号优先", preferFreeBuildHelp: "启用后优先选择已确认可用的 Free 账号，其他路由条件保持不变。" },
        audit: { title: "请求审计", bufferSize: "队列容量", bufferSizeHelp: "审计记录进入持久化前可在内存队列中等待的最大数量。", batchSize: "批量写入数", batchSizeHelp: "每次数据库事务最多写入的审计记录数量。", flushInterval: "刷新间隔", flushIntervalHelp: "队列未达到批量写入数时，强制提交待处理审计记录的间隔。", commitDelay: "提交聚合等待（毫秒）", commitDelayHelp: "需要确认提交的请求进入原子批次前允许等待的最长时间。" },
      ...
      apiErrors: {
        ...
        quotaResetRedeemFailed: "重置 Grok Web 额度失败",
  ...
  en: {
    translation: {
      ...
      webQuotaRedeem: { action: "Reset quota", completed: "Quota reset request completed" },
      ...
      audits: {
        ...
        billingStoredFormulaUnavailable: "This record uses a historical or unknown rate version. Its final estimate is retained, but the formula cannot be reconstructed reliably with current rates.",
      ...
      apiErrors: {
        ...
        quotaResetRedeemFailed: "Failed to reset the Grok Web quota",
